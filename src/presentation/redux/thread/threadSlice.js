...
import AllThreadsUseCase from "../../../application/thread/GetAllThreadsUseCase.js";
import ThreadDetailUseCase from "../../../application/thread/GetThreadDetailUseCase.js";
import CommentUseCase from "../../../application/thread/SubmitCommentUseCase.js";
...
import { UpVoteThreadUseCase } from "../../../application/thread/UpVoteThreadUseCase.js";
import { DownVoteUC } from "../../../application/thread/DownVoteUC";
import { NeutralVoteThreadUseCase } from "../../../application/thread/NeutralVoteThreadUseCase.js";
import { UpVoteCommentUseCase } from "../../../application/thread/UpVoteCommentUseCase.js";
import { DownVoteCommentUseCase } from "../../../application/thread/DownVoteCommentUseCase.js";
import { NeutralVoteCommentUseCase } from "../../../application/thread/NeutralVoteCommentUseCase.js";
...
export const submitThread = createAsyncThunk(
  ...
  async (threadData, { rejectWithValue }) => {
    try {
      ...
      const submitThreadUseCase = new SubmitThreadUseCase(
        threadRepository,
        authRepository,
      );
  ...
  async (threadId, { rejectWithValue, getState }) => {
    try {
      ...
      if (thread && thread.isUpVotedByCurrentUser) {
        ...
        const currentUserId = getState().auth.user
...
export const neutralVoteComment = createAsyncThunk(
  ...
  async ({ threadId, commentId }, { rejectWithValue, getState }) => {
    try {
      ...
      const result = await neutralVoteCommentUseCase.execute(
        threadId,
        commentId,
      );
      if (result === false) {
    ...
    builder
      ...
      .addCase(fetchThreads.fulfilled, (state, action) => {
        ...
        const uniqueCategories = [
          ...new Set(action.payload.map((thread) => thread.category)),
        ];
      ...
      .addCase(upVoteThread.pending, (state, action) => {
        ...
        if (state.detailThread && state.detailThread.id === threadId) {
          const newUpVotesBy = state.detailThread.upVotesBy.includes(
            currentUserId,
          )
      ...
      .addCase(downVoteThread.pending, (state, action) => {
        ...
        if (state.detailThread && state.detailThread.id === threadId) {
          const newDownVotesBy = state.detailThread.downVotesBy.includes(
            currentUserId,
          )
            ? state.detailThread.downVotesBy.filter(
                (id) => id !== currentUserId,
              )
      ...
      .addCase(downVoteThread.fulfilled, (state, action) => {
        ...
        if (state.detailThread && state.detailThread.id === threadId) {
          const newDownVotesBy = state.detailThread.downVotesBy.includes(userId)
          ...
          state.detailThread = {
            ...
            comments: state.detailThread.comments.map((comment) => {
              if (comment.id === commentId) {
                ...
                const originalUpVoted =
                  comment.upVotesBy.includes(currentUserId);
                const originalDownVoted =
                  comment.downVotesBy.includes(currentUserId);

                let newUpVotesBy = comment.upVotesBy.filter(
                  (id) => id !== currentUserId,
                );
                let newDownVotesBy = comment.downVotesBy.filter(
                  (id) => id !== currentUserId,
                );
...
export const selectDownVote = (state) => state.threads.downVoteStatus;
